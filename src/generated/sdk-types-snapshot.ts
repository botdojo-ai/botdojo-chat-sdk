 ...
 * 
 * Generated: 2025-12-28T14:24:46.719Z
 */
...
// ============================================================================
// Core Backend Type Stubs
// These types are simplified stubs for the sdk-chat bundle.
// The actual backend types are more complex but these suffice for the SDK interface.
// ============================================================================

/** Flow canvas configuration (stub type) */
export interface Core_FlowCanvas {
  id?: string;
  type?: string;
  data?: any;
  [key: string]: any;
...
/** Chat side panel configuration (stub type) */
export interface Core_IChatSidePanel {
  [key: string]: any;
}

/** Citation reference (stub type) */
export interface Core_Citation {
  id?: string;
  source?: string;
  text?: string;
  url?: string;
...
/** Tool execution phase (stub type) */
export type Core_ToolExecutionPhase = 'pending' | 'executing' | 'completed' | 'error' | string;

/** Token update event (stub type) */
export interface Core_TokenUpdate {
  token?: string;
  content?: string;
  [key: string]: any;
}
 ...
 */
export function extractToolHandlers(tools: ToolDefinition[] | Record<string, ToolDefinition>): Record<string, ToolHandler> {
...
// From: modelContextTranslation-snapshot.ts
/**
 * Model Context Translation Snapshot
 * AUTO-GENERATED by scripts/sync-model-context-translation.js
 * DO NOT EDIT MANUALLY
 * 
 * This file is a build-time copy of modelContextTranslation from botdojo-core
 * It does NOT create a package dependency.
 * 
 * Source: botdojo-core/src/utils/modelContextTranslation.ts
 * To update: pnpm sync-model-context-translation
 */

/* eslint-disable */

/**
 * Model Context Translation Utilities
 * 
 * Converts SDK ModelContext format (plain objects) to backend CanvasModelContext format.
 * This is the single source of truth - sdk-types will sync this via prebuild script.
 * 
 * Note: This function accepts `any` to avoid importing SDK types and creating circular dependencies.
 * The synced version in sdk-types will wrap this with proper SDK types.
 */

/**
 * Backend format for model context (what the API expects via RPC)
 * These types mirror the CanvasModelContext structure the backend expects when receiving model context.
 */
export interface BackendModelContext {
    ...
    toolPrefix?: string;
    resourceUri?: string;
    tools: BackendToolDef[];
    prompts: BackendPromptDef[];
    resources?: BackendResourceDef[];
}
...
export interface BackendToolDef {
    ...
    _meta?: any;  // Preserve _meta for additional metadata (e.g., streaming-preview)
}

export interface BackendPromptDef {
    name: string;
    description?: string;
    messages: Array<{
        role: string;
...
export interface BackendResourceDef {
    uri: string;
    name?: string;
    description?: string;
    mimeType?: string;
 ...
 * 
 * @param context - The SDK ModelContext from consumer (accepts any to avoid SDK type dependency)
 * @returns Backend format ready to send via API/RPC (CanvasModelContext format)
 */
export function translateModelContextToBackend(
    context: any
): BackendModelContext {
    // Build tools array (note: backend expects 'tools' not 'toolDefs')
    const tools = translateToolsToBackend(context.tools || []);
    
    // Build prompts array (note: backend expects 'prompts' not 'promptDefs')
    const prompts = translatePromptsToBackend(context.prompts || []);
    
    // Build resources array (note: backend expects 'resources' not 'resourceDefs')
    const resources = translateResourcesToBackend(context);
    
    return {
        name: context.name,
        description: context.description,
        toolPrefix: context.toolPrefix,
        resourceUri: context.uri,
        tools,
        prompts,
        resources
    };
...
/**
 * Convert SEP-1865 MCP Apps resource template metadata to canvasTemplate format
 * 
 * SEP-1865 UI metadata lives under metadata.ui:
 * - preferredFrameSize: { width, height }
 * - initialRenderData: initial state object
 * - displayName: display name for the canvas
 * - canvasId: canvas identifier (optional)
 * - stateless: boolean (if true, canvas is never persisted to DB)
 * 
 * BotDojo canvasTemplate format (generated):
 * - type: 'mcp-app'
 * - width: string (e.g., '100%')
 * - height: string (e.g., '520px')
 * - stateless: boolean (if true, canvas is not persisted to database)
 * - initialData: { url, show_inline: true, agent_enabled: true, ...initialRenderData }
 * 
 * Note: 
 * - show_inline and agent_enabled are always true for MCP Apps canvases
 * - The same template is used for both streaming preview and final canvas
 * - Progressive updates during streaming are done via updateCanvasData(), which patches the initialData with new state
 */
function convertUiResourceTemplateToCanvasTemplate(resourceTemplate: any): any {
    if (!resourceTemplate || !resourceTemplate.uri) {
        return undefined;
    }
    
    const metadata = resourceTemplate.metadata || {};
    const uiMeta = metadata.ui || {};

    const frameSize = uiMeta.preferredFrameSize || {};
    const width = Array.isArray(frameSize) ? frameSize[0] : frameSize.width || '100%';
    const height = Array.isArray(frameSize) ? frameSize[1] : frameSize.height || '520px';

    const initialRenderData = uiMeta.initialRenderData || {};
    const displayName = uiMeta.displayName;
    const stateless = uiMeta.stateless === true;
    const mimeType = resourceTemplate.mimeType;
    const html = uiMeta.html;
    const prefersProxy = uiMeta.prefersProxy;
    const allowExternal = uiMeta.allowExternal;
    const baseUrl = uiMeta.baseUrl;
    const urlList = uiMeta.urlList;
    ...
    return {
        ...
        stateless,
        initialData: {
            url: resourceTemplate.uri,
            show_inline: true,
            agent_enabled: true,
            ...(mimeType && { mimeType }),
            ...(html && { html, inlineHtml: true }),
            ...(prefersProxy !== undefined && { prefersProxy }),
...
export function translateToolsToBackend(
    ...
    const toolsArray = Array.isArray(tools) 
        ? tools 
        : Object.entries(tools || {}).map(([name, tool]: [string, any]) => ({ ...tool, name }));
    
    // Map to backend format (CanvasModelContextTool expects 'arguments' field)
    return toolsArray.map((tool: any) => {
        // Priority order for canvasTemplate:
        // 1. Explicit canvasTemplate field (highest priority)
        // 2. _meta['botdojo/canvas'] (BotDojo-specific format)
        // 3. _meta['botdojo/resource-template'] (UI resource template - convert to canvasTemplate)
        ;
        let canvasTemplate = tool.canvasTemplate || tool._meta?.['botdojo/canvas'];
        
        // Convert SEP-1865 UI resource template to canvasTemplate if present
        if (!canvasTemplate && tool._meta?.['botdojo/resource-template']) {
            canvasTemplate = convertUiResourceTemplateToCanvasTemplate(
                tool._meta['botdojo/resource-template']
            );
        }

        // Support SEP-1865 ui metadata on tools without the BotDojo resource-template wrapper
        if (!canvasTemplate && tool._meta?.ui?.resourceUri) {
            canvasTemplate = convertUiResourceTemplateToCanvasTemplate({
                uri: tool._meta.ui.resourceUri,
                mimeType: tool._meta.ui.mimeType || 'text/html;profile=mcp-app',
                metadata: { 
                    ui: tool._meta.ui, 
                    ...(tool._meta?.['botdojo/no-cache'] === true ? { 'botdojo/no-cache': true } : {})
                }
            });
        }
        
        // Copy botdojo/no-cache from tool._meta to canvasTemplate metadata (only if true)
        if (canvasTemplate?.initialData?.metadata && tool._meta?.['botdojo/no-cache'] === true) {
            canvasTemplate.initialData.metadata['botdojo/no-cache'] = true;
        }
        
        return {
            name: tool.name!,
            description: tool.description,
            displayName: tool.displayName || tool._meta?.['botdojo/display-name'] || tool._meta?.ui?.displayName || tool.name!,
            // Convert SDK's inputSchema (MCP standard) to backend's arguments field
            // Keep fallback for legacy: arguments > parameters > inputSchema
            arguments: tool.arguments || tool.parameters || tool.inputSchema,
            canvasTemplate,
            hideStep: tool._meta?.['botdojo/hide-step']??false,
            hideStepDetails: tool._meta?.['botdojo/hide-step-details']??false,
            // Preserve _meta for additional metadata (e.g., streaming-preview, execute function)
            _meta: tool._meta
        };
    });
}
...
export function translateResourcesToBackend(
    ...
    const resources: BackendResourceDef[] = [];
    
    // Add resources from unified resources array
    if (context.resources) {
        resources.push(...(context.resources || []).map((r: any) => {
            // Check if dynamic (has uriTemplate) or static (has uri)
            const uri = 'uriTemplate' in r ? r.uriTemplate : r.uri;
            return {
                uri,
                name: r.name,
...
// Excluded: converters.ts (not needed for @botdojo/chat-sdk)
...
// Excluded: type-mappers.ts (not needed for @botdojo/chat-sdk)

// From: backend-types-snapshot.ts
/**
 * Backend Types Snapshot
 * AUTO-GENERATED by scripts/sync-backend-types.js
 * DO NOT EDIT MANUALLY
 * 
 * This file is a build-time snapshot of types from botdojo-sdk-backend-types
 * for validation purposes. It does NOT create a package dependency.
 * 
 * To update: pnpm sync-backend-types
 * Generated: 2025-12-27T11:57:43.243Z
 */

/* eslint-disable */

// Re-export types from compiled backend-types package
// This creates a validation snapshot without a package dependency
// Note: Model context types are NOT included here as SDK handles translation
